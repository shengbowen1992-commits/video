...
适用工作流：`H3-动态多段-方案5-无Video-OpeningState-576x960.json`。节点从女主参考图文件夹读取文件名主干恰好为 `picture01` 至 `picture05` 的五张图片（支持 PNG、JPG、JPEG、WEBP）；文件夹默认路径为 `F:\video\素材\2026-09-25-棕金礼服人物参考-v1`。男主图片仍由工作流中的单张图片节点提供。启动任务时六张图都会复制到该任务的 `refs` 文件夹，重做旧任务使用当时的快照。
