...
- `<Picture 1>`、`<Picture 2>` 始终保持各自已分配的身份参考角色，不把第二张身份图变成尾帧。角色的身份、性别和实际人数来自用户要求及输入，不由文件格式规定。
- 第 1 段没有上一段视频，不虚构 `<Video 1>`。
- 第 2 段开始，`<Video 1>` 始终指向紧邻上一段的尾部视频。第 3 段的 `<Video 1>` 是第 2 段尾部，第 6 段的 `<Video 1>` 是第 5 段尾部。
- JSON 的 `id` 递增，引用输入编号 `<Video 1>` 不递增。不能改写为 `<Video 2>`、`<Video 3>`。
- 视频带有音轨不代表已启用参考音频输入；仅在实际绑定了音频参考时按原规范定义 `<Audio N>`。
- 这是一套输入绑定说明，不是硬性首帧锁定或跨段连续性的保证。既有旧格式使用的尾帧图片和这里的尾部视频不能未经确认互换。
