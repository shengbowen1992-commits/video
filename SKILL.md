...
Resolve reference numbers from the selected workflow before writing prompts. In the original two-image dynamic workflow and Plan 6, `<Picture 1>` and `<Picture 2>` retain their assigned identities. The original workflow uses `<Video 1>` from segment 2 onward; Plan 6 is hybrid and binds it only on video-reference segments. Where bound, `<Video 1>` means the immediately preceding tail video and its number does not increase with `id`. In **Plan 5 (No Video / Opening State)**, `<Picture 1>`–`<Picture 5>` are five views of the same female lead and `<Picture 6>` is the male lead; no segment receives `<Video 1>`. See [the workflow-specific mapping](references/story-segments-json.md#方案五五张女主参考图的专用编号). Do not infer a cast count from the number of pictures, relabel an identity image as a tail, or mechanically replace tags in existing user text. Video reference conditioning does not guarantee literal first-frame equality.
...
- The legacy picture layout below assumes one permanent identity image and one tail image. Dynamic-series bindings depend on the selected workflow: use the two-image mapping with video only where bound for the original workflow and Plan 6, or the six-image, no-video mapping for Plan 5. When the user supplies multiple independent identities, reserve a distinct label for each image and a separate label for the tail; apply the reference's multi-identity mapping instead of assigning two incompatible roles to `<Picture 2>`.
...
- Ref2VA prompts keep permanent face identity and previous-tail geometry on separate, correctly typed references; video-reference dynamic-series continuations use `<Video 1>` without incrementing its number, while Plan 5 uses no video reference.
